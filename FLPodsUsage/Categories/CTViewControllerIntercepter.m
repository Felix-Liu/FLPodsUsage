...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [UIViewController aspect_hookSelector:@selector(testMethod) withOptions:AspectPositionAfter usingBlock:^{
            NSLog(@"testMothod");
        } error:NULL];
        
        [UIViewController aspect_hookSelector:@selector(prefersStatusBarHidden) withOptions:AspectPositionInstead usingBlock:^() {
            NSLog(@"%@", NSStringFromSelector(@selector(prefersStatusBarHidden)));
        } error:NULL];
        
        Class metalClass = objc_getMetaClass(NSStringFromClass([UIImage class]).UTF8String);
        [metalClass aspect_hookSelector:@selector(imageNamed:) withOptions:AspectPositionAfter usingBlock:^(id<AspectInfo> info, NSString *name){
            NSLog(@"%@", name);
        } error:NULL];
    }
    return self;
